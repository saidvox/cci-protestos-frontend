...
export const apiService: AppService = {
  ...
  async uploadOfficialDocument(input) { const form = new FormData(); form.append("titulo", input.title); if (input.description) form.append("descripcion", input.description); if (typeof input.order === "number") form.append("orden", String(input.order)); form.append("file", input.file); return adaptOfficialDocument((await mutate(() => apiClient.post<OfficialDocumentDto>("/documentos-tramite", form))).data) },
  async deactivateOfficialDocument(id) { return adaptOfficialDocument((await mutate(() => apiClient.delete<OfficialDocumentDto>(`/documentos-tramite/${id}`))).data) },
  ...
  async createAnalyst(input) { return adaptAnalyst((await mutate(() => apiClient.post<AnalystDto>("/analistas", { nombre: input.name, email: input.email, codigo: input.code, entidadId: input.entityId }))).data) },
