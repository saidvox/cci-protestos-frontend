...
export interface RequestDto { id: number; codigo: string; solicitante: string; entidad: string; analista: string | null; estado: RequestStatus; motivo: string; observacion: string | null; creadoEn: string; actualizadoEn: string }
...
export const adaptRequest = (dto: RequestDto): RequestRecord => ({ id: dto.id, code: dto.codigo, applicant: dto.solicitante, financialEntity: dto.entidad, type: dto.motivo.split(" | ")[0] || "Solicitud", status: dto.estado, createdAt: dto.creadoEn, observation: dto.observacion ?? undefined })
